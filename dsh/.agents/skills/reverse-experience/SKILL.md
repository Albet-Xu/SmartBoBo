...
本技能与 `reverse-memory` MCP server 配套使用。模型通过 `mcp__reverse-memory__reverse_memory_*`
工具读写记忆库；本技能说明**何时**查、**何时**存、怎么组织内容，以及红线约束。
（逆向模式挂载名为 `reverse-memory`，工作流模式挂载名为 `reverse-memory-wf`，工具前缀随之变化，
工具名一致，都是 `reverse_memory_*`。）
...
> 注意：逆向模式与工作流模式挂载的都是**读写**实例（服务端注册全量写工具，
> 可 save/feedback/cleanup；工作流模式就地逆向后同样能沉淀经验）。只读（`REVERSE_MEMORY_READONLY=1`）
> 仅作为兜底配置存在，默认不启用。

## 何时查（逆向/工作流模式通用）

1. **逆向新站点、且无法从域名直接判断套路时**：开始动手前提 `reverse_memory_search`——
   传目标域名 + 已观察到的特征（反爬 SDK 名、签名参数名、混淆特征、412/403 现象等）。
2. **卡在某一步时**：用报错/特征词再查一次，看历史有没有同款坑。
3. `reverse_memory_search` 结果 `mode=local` 表示 Qdrant 不可达、已降级为本地检索，结果仍可用但
   覆盖面较小。
