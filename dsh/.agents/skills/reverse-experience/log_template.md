...
| `confidence` | 置信度 **1-5**（连续值，可带 1 位小数）。**只有严格大于 2.0 才入库**；≤ 2.0 直接放弃。锚点：一次跑通=2.0、复现两次以上=2.5、跨时间稳定=3.5-5 |
...
- 失败日志同样宝贵：`result=FAIL` 的日志照常沉淀（置信度严格大于 2.0 即可），
  明确写出「试过的方法哪些无效、卡在哪」。
- 敏感信息打码：不写入真实 Cookie / Token / 密钥原文，用 `<cookie>`、`<sk>` 等占位。
- 只写本次任务实际发生的事，不写猜测；置信度如实自评（一次跑通=2.0（会被丢弃），
  复现两次以上才给 2.5 以上）。