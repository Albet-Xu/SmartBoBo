...
每次符合条件的 `agent/pre-step`，该插件都会使用调用会话的 cwd 调用 `ctx.skills.snapshot()`，将 pre-step 中止信号转发到发现流程，应用 `skill` 工具的精确可见性，并按顺序渲染 `name` 和 `description` 条目。如果先前不存在目录且该视图非空，插件会向下游 `enter` 决策添加初始的持久用户角色 `<system-reminder>`。目录消息只包含这些摘要；skill 正文、路径、来源、提供方和 `whenToUse` 提示仍位于目录之外。skill 可在共享摘要中声明 `triggers` 列表：当存在且非空时，仅当请求的正规化直接用户文本中包含其中一个短语（对大小写与空白不敏感的子串匹配）才显示该条目；否则与以往一样常驻列出。该门槛基于本步的直接用户消息求值，因此可见集合（以及目录 digest）会随主题在跨步之间变化。
...
无法解析的名称会报告 skill 未知或已不可用。无效名称和 `invocation.modelInvocable` 为 `false` 的 skill 会产生不同的错误结果。`invocation.userInvocable` 不限制这个面向模型的接口。`triggers` 列表非空的 skill 要求会话表面上的最新直接用户消息命中其中一个触发词；加载器会在目录门槛之外，用请求文本再次校验该门槛，因此在主题不匹配时无法加载被门槛约束的 skill，`skill` 工具也会返回各自不同的"触发词门槛未命中"错误。
...
- **受触发词约束的目录会跟随主题**：当请求不再命中某被门槛约束技能的触发词时，该技能会从下一次广告目录中移除并追加一次替换；保持同一主题时 digest 稳定，但切换主题时每次变更都要花费一次替换的成本。
