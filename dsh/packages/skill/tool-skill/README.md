...
At every eligible `agent/pre-step`, the plugin calls `ctx.skills.snapshot()` for the calling session's cwd, forwards the pre-step abort signal to discovery, applies exact `skill` tool visibility, and renders the ordered `name` and `description` entries. When no prior catalog exists and that view is non-empty, it adds an initial durable user-role `<system-reminder>` to a downstream `enter` decision. Catalog messages contain only those summaries; skill bodies, paths, sources, providers, and `whenToUse` hints remain outside the catalog. A skill may declare a `triggers` list in the shared summary: when present and non-empty, the entry appears only for requests whose normalized direct-user text contains one of the phrases (case- and spacing-insensitive substring match); otherwise it is listed as before. The gate is evaluated against the step's direct user messages, so the visible set (and the catalog digest) follows the topic across steps.
...
An unresolved name reports that the skill is unknown or no longer available. Invalid names and skills whose `invocation.modelInvocable` is `false` produce distinct error results. `invocation.userInvocable` does not restrict this model-facing tool. A skill with a non-empty `triggers` list requires the latest direct user message on the session surface to match one trigger; the loader rechecks this gate against the request text behind the catalog, so a gated skill cannot be loaded while the topic does not match, and the `skill` tool reports a distinct gated error.
...
- **Trigger-gated catalogs follow the topic** — a request that stops matching a gated skill's triggers removes it from the next advertised catalog, appending a replacement; staying on one topic keeps the digest stable, but switching topics costs one replacement per change.
