  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      function triggerLoadOlderIfPresent() {
        var olderBtn = document.querySelector('[class*="older"] button');
      ...
      // Adapted to the DSH fork: conversation nodes are located by the stable
      // data-chat-flow-kind attribute (ChatNodeSeat renders
      // <div data-chat-flow-kind={node.kind}>), not by minified class names.
      function assistantTextOf(item) {
        var exclusions = '[data-variant="think"], [class*="reasoning"], [class*="Reasoning"], [class*="tool"], [class*="Tool"], [class*="callRow"], [data-turn-tail], [class*="deliverables"], [class*="action"], [class*="Action"]';
        var mdSelectors = '[class*="MarkdownText"], [class*="markdown"], [class*="body"]';
        var mdContainers = Array.from(item.querySelectorAll(mdSelectors));
        var candidateText = '';

        for (var mIdx = 0; mIdx < mdContainers.length; mIdx++) {
          var md = mdContainers[mIdx];
          if (md.closest(exclusions)) continue;
          var txt = md.innerText ? md.innerText.trim() : '';
        ...
        if (!candidateText) {
          var cleanClone = item.cloneNode(true);
          Array.from(cleanClone.querySelectorAll(exclusions + ', button, time')).forEach(function (el) { el.remove(); });
          candidateText = cleanClone.innerText ? cleanClone.innerText.trim() : '';
        }
        return candidateText;
      }

      function collectTurns() {
        var messageNodes = Array.from(document.querySelectorAll('[data-chat-flow-kind]'));
        ...
        for (var i = 0; i < messageNodes.length; i++) {
          ...
          if (item.classList.contains('dsh-ht-fixed-rail-container')) continue;

          var flowKind = item.getAttribute('data-chat-flow-kind') || '';

          // Tail / deliverables / workflow rows close the open turn's range.
          ...
          var isUser = (flowKind === 'user' || flowKind === 'steering');
          if (isUser) {
            var userBubble = item.querySelector('[class*="userRow"] [class*="bubble"], [class*="bubble"]');
            var cleanUserText = '';
            if (userBubble) {
              cleanUserText = userBubble.innerText ? userBubble.innerText.trim() : '';
            } else {
              var cleanClone = item.cloneNode(true);
              Array.from(cleanClone.querySelectorAll('button, time, [class*="action"], [class*="time"], [class*="icon"]')).forEach(function (el) { el.remove(); });
              cleanUserText = cleanClone.innerText ? cleanClone.innerText.trim() : '';
            }
          ...
          // Other rows (assistant step, tool, command, compaction, ...) extend the open turn.
          if (currentTurn) {
            currentTurn.endElement = item;

            if (flowKind === 'assistant-step' && !currentTurn.assistantText) {
              var candidateText = assistantTextOf(item);
              if (candidateText) {
