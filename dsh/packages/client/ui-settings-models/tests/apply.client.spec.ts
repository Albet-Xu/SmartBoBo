...
describe('ui-settings-models apply', () => {
  ...
  it('registers the models nav entry for declarations before or after apply', async () => {
    ...
    // The fork's apply only registers the Models section; product onboarding
    // (opencode-go) is driven by welcome-store + OnboardingModal, not as a
    // settings.onboarding slot entry.
  ...
  it('the models section inject face wires the controller and live snapshot', async () => {
    ...
    expect(typeof injected.useSnapshot).toBe('function')
    expect(injected.t('nav')).toBe('模型')
    // The store starts idle; refreshIfLoaded is a no-op until the first load,
    // which keeps background invalidations from triggering an unused fetch.
...
describe('pushed invalidations', () => {
  ...
  it('routes pushed credential invalidation into the models page refresh', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('settings.section')[0]!
  ...
  it('routes every pushed invalidation source into the models page refresh', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('settings.section')[0]!
    ...
    // Unrelated events must not trigger a refresh.
    b.ctx.remote.$dispatch('some/other-event', [])
    expect(load).toHaveBeenCalledTimes(4)
  })
})
